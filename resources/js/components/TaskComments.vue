            ...
            <ul class="list-unstyled">
                <template v-if="comments.length > 0">
                    <li v-for="(c, i) in comments" :key="i" class="comment-item p-2 mt-2">{{
                            c.body }} <em class="fa fa-trash text-danger mx-2 float-right" title="Delete comment" style="cursor: pointer; font-size: 15px"></em> </li>
                </template>
...
export default {
  ...
  setup(props) {
    ...
    const {
        comments,
      getComments, loading, form, validateErrors, createComment,
